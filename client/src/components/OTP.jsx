...
const OTP = ({ email, name, setOtpVerified, reason }) => {
  ...
  const sendOtp = async () => {
    if (hasSentInitialOtp.current) {
      console.log("Preventing duplicate OTP send");
      return; // Prevent sending if already sent once
    }
  ...
  useEffect(() => {
    ...
    sendOtp(); // This will only send if hasn't been sent before due to the check in sendOtp
  }, []);

  const resendOtp = () => {
    setShowTimer(true);
    clearTimer(getDeadTime());
    hasSentInitialOtp.current = false; // Reset the flag to allow resend
    sendOtp();
  };
